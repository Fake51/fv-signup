<?
 class wear_v1
 {
        ...
        function ft_renderWearJSON($json,$usertype)
        {
            $language = "da";
            $user = 
        	...
        	foreach($my_wear as $wear)
        	{
    			...
    			renderFieldByType(array(
        	     ...
        	     $sizearray = array("XS","S","M","L","XL","2XL","3XL","4XL","5XL","6XL","7XL","8XL","9XL","10XL");
        	     ...
        	     for ($i=array_search($parts[0],$sizearray);$i<=array_search($parts[1],$sizearray);$i++)
        	     {
     	    	 ...
     	    	 if ($size_count>1)
    	    	 {
        	    	$select_values = array('' => __tm('Størrelse'));
        		    for ($i=array_search($parts[0],$sizearray);$i<=array_search($parts[1],$sizearray);$i++)
        		    {
	...
	function render()
	{
    	...
    	$url = "api/wear/*?brugertype=".$this->brugertype;
